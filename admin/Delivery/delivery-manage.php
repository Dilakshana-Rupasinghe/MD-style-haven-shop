...
if (!empty($status_filter) && !empty($search_customer)) {
    $get_itemDetails = "SELECT * FROM `order` 
                        ...
                        ORDER BY order_status = 'Ready for Delivery' DESC, order_id DESC";
    ...
    $get_itemDetails = "SELECT * FROM `order` 
                        WHERE order_status IN ('Ready for Delivery', 'Complete') 
                        ORDER BY order_status = 'Ready for Delivery' DESC, order_id DESC";
}
...
if (isset($_GET['CompleteId'])) {
    $order_id = ($_GET['CompleteId']);

    $updateOrder = "UPDATE `order` SET order_status = 'Complete', order_deliver_date = NOW() WHERE order_id = '$order_id'";
    ...
    echo "<script>window.open('delivery-manage.php', '_self');</script>";
}
                ...
                <form method="GET" class="mb-3">
                    <div class="row g-3 align-items-center">
                        ...
                        <div class="col-auto d-flex">
                            ...
                            <a href="delivery-manage.php" class="btn btn-secondary ms-3">Reset</a>
                        </div>
                    </div>
                </form>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        if ($row_count == 0) {
                            ...
                            <a href='delivery-manage.php?CompleteId=" . $row['order_id'] . "' class='$invisible update'>Complete</a> ";
                                echo "</tr>";
                            }
                        }
                        ?>
